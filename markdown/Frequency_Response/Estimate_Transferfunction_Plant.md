...
\[
G_{yu}(j\omega)= \frac{P(j\omega)}{1 + P(j\omega)C_{D}(j\omega)G_{fD}}, \qquad
G_{yw}(j\omega) = T(j\omega) = \frac{G_{yu}(j\omega)\,C_{PI}(j\omega)}{1 + G_{yu}(j\omega)\,C_{PI}(j\omega)}, \qquad
...
\[
P(j\omega) = P_\text{real}(j\omega) \cdot G_f(j\omega),
\]
where \( G_f(j\omega) \) denotes the gyro filter dynamics.
...
They are defined as
\[
S_{UU}(j\omega) = U(j\omega) \cdot \overline{U(j\omega)}, \qquad
S_{YU}(j\omega) = Y(j\omega) \cdot \overline{U(j\omega)} \qquad
S_{YY}(j\omega) = Y(j\omega) \cdot \overline{Y(j\omega)}
\]

Using these spectra, the **magnitude-squared coherence** is obtained as
\[
\gamma^2(f) = \frac{|S_{YU}(f)|^2}{S_{YY}(f)\,S_{UU}(f)}.
\]
