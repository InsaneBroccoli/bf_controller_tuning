...
To reduce the influence of noise and disturbances, the functions [`apply_rotfiltfilt`](#) and [`estimate_frequency_response`](#) were used.  
...
Additionally, care was taken to use as few signals as possible from the closed loop,  
since noise in those signals is fed back and can therefore be amplified.  
...
The individual transfer functions are related by the closed-loop identities:
\[ G_{yu}(s)= \frac{P(s)} {1+P(s)C_{D}(s)}, \qquad G_{yw}=T(s) = \frac{G_{yu}(s)\,C_{PI}(s)}{1 + G_{yu}(s)\,C_{PI}(s)}, \qquad G_{uw}(s) = \frac{C_{PI}(s)}{1 + G_{yu}(s)\,C_{PI}(s)} \] \[ \Rightarrow P(s) = \frac{T(s)}{G_{wu}(s)}. \]

This allows the plant \( P(s) \) to be reconstructed from closed-loop data without opening the control loop during measurement.
