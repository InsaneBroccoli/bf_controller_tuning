...
The function **`apply_rotfiltfilt`** uses a **time-dependent phasor** $p(t) = e^{i\,\text{sinarg}(t)},$where `sinarg` represents the **phase progression** of the carrier signal – for example, the phase of a **chirp** (a signal whose frequency changes continuously over time).  
...
$y_R(t) = x(t)\,p(t),$ $\qquad$ $\qquad$ $y_Q(t) = x(t)\,p^*(t),$
...
$x_f(t) = \text{Re}{ \frac{1}{2}\big( y_R(t)\,p^*(t) + y_Q(t)\,p(t) \big)}$$
