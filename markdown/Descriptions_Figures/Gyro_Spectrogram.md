...
The spectrograms show how the gyro noise is distributed over frequency and throttle for all three axes (Roll, Pitch, Yaw), once without and once with filtering. This makes the plots very useful to understand how the drone behaves over the whole throttle range and how effective the filters are.
...
- **Where the nouse band appear and how they move with throttle.** This will help you to identifiy the frequency ranges that filters must convert.
...
- **Where to set the RPM filter range** because the spectrograms show the frequency of the motor harmonics at different throttle levels.
