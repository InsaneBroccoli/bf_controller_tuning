...
Throughout the project, we achived a range of practical results and technical results. The central and most important result was that we gained a deep understanding of the Betaflight control system, inculding how its filters structures and controller components interact to stabilise the gyro control loop.In addition, we developed a solid foundation in the application of essential signal-processing methods and learned how to use them correctly for frequency-domain analysis, step-response estimation, and data preparation. This newly lerned knowledge will be crucial for our future development of the tool and form an important basis for the work that will follow in the bachelor’s thesis. The simulations carried out during the project helped us link theoretical concepts with the real dynamic behaviour of an FPV drone and validate our understanding of the control loops.
Next to the theoretical knowledge, we also built and configurated an FPV drone form scratch and performed flight test and tuning on it. This hands-on experience was a valuable insight into the practical challenges of drone construction, setup, flight and tuning. It helped us to better understand the real-world implications of our analysis and the needs of the FPV community and helped us to bridge the gap between theory and practice.
On the software side, we restructured the existing codebase into a modular format, which will allow us to add new features and improvements more easily in the future. We also gained experience in object-oriented programming and code organization for larger projects. Additionally, we made significant progress in using MATLAB and Python for control engineering applications, even though the full transfer to Python is still ongoing. Overall, these results provide a strong foundation for further development and refinement of the tool in our bachelor’s thesis.
A time-consuming part of the project was the development of an automatic extraction method for flight data directly from Betaflight Blackbox logs. Despite different approaches, we were unable in the time frame, to complete a reliable solution. However, the insights gained during this work form a valuable basis for further investigations, and the topic will be revisited as part of the bachelor’s thesis.
In summary, the project demonstate that offline analysis and tuning of Betaflight FPV drones provieds a rubust, objective and data-driven approach to optimize flight performance. The implemented methods enable the reconstruction of control-loop for flight measurements, allowing for a detailed understanding of system dynamics and the impact of different controller settings. The wokr carried out so far lays a solid foundation for the bachelor's thesis, during which the tool will be extended with additional tuning capabilities, migrated to Python, and validated through further flight tests.
...
## 6.2 Next steps
This project is being continued and further developed as part of a bachelor’s thesis. As part of this, we would like extend the existing codebase with new features such as the offline tuning for position and altitude hold. This should give us the insight into how these control loops behave and how we can optimize them.
Additionally, the full transfer from MATLAB to Python is planned. This will make the tool more accessible to the FPV community, as Python is open source and free to use. Our goal is that the tool can be easily integrated into existing Betaflight tools, so that pilots can use it for offline tuning of their drones.
Finally, we want to validate the analysis results with more flight tests. By comparing the offline analysis with real-world flight behaviour, we can ensure that our methods are accurate and reliable. This will help us to further refine the tool and make it more useful for the FPV community.
