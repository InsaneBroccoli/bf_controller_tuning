...
In addition to the MATLAB code, we created a Simulink simulation, in which we tested the step response of our simulation of the Betaflight control system. The simulated answer got close to the step response of the real drone. For us, this was the confirmation, that we got the controller order correct.

### 4.1.2 Simulation PID Controller

To deepen our understanding of the different controller components, we created an additional MATLAB script. This script allows us to combine different controller types with simple plant models. The program allowed us to switch between P, I, PI, PD and PID controllers, and between basic plant types. Through this, we were able to observe how each controller component influences the overall system behaviour.
The script calculates the open and closed loop of the chosen settings and generates Bode plots, Nyquist diagrams and step respones. This helped us to understand and visuallise the effect of each component. Trough that, our overall understanding of contoller behaviour significantly improved.

### 4.1.3 Simulation Frequency Response

To analyse the frequency response of a system based on input and output data, we created a additional simulation. In this simulation we created a chirp signal on which we added noise. We added the noise to simulate more realisic measurement conditions. Using these signals, we implemented different methods to estimate the system’s transfer function.
First, we calculated a basic frequency response using the plain FFT. That got us a good benchmark. After that, we applied different signal processing methods. First we used the Welch methode to improve the estimate by averaging over overlapping segments. The implementation of the Welch FRF follows the theory described in the documentation [Estimate Frequency Response](../Frequency_Response/estimate_frequency_response.md). This allowed us to compare the plain FFT approach with the more robust Welch estimator.
In the next step we implemented a rotated-signal approach, wher the chirp is demodulated to baseband before filtering. For this part we used the zero-phase filtering method apply_rotfiltfilt, as described in the documentation [Apply Rotfiltfilt](../Frequency_Response/ApplyRotfiltfilt.md). After that we compared the orignal bode plot to the signal processed one, to see how the rotation technique improves the noise robustness.
Overall, this script helped us understand how strongly noise affects the estimated transfer function and how we can increase the robustness of the signals.
...
Overall, this exercise helped us understand how windowing, overlap and segment length affect the spectral estimate, and how these methods can increase robustness and reduce spectral leakage.

### 4.1.5 Simulation Spectogram

For better understandig of the analyse how the frequency content of the excitation signal changes with both time and thrust, we developed an additional MATLAB simulation. In this script, we generate a noisy sinusoidal input signal and a slowly varying thrust signal. The input signal is then split up in different segments and windowed with a Hann windwo. After this the signal is transformad using the FFT in order to obtain a time-frequency repesentation.
At first, we arranging the segment spectra along the time axis and after that, we along the thrust levels. So we get two spectroframs, one is showing the amplitude over frequency and time, and the other one is showing us the amplitude over frequency and thrust. Through this visualisation, we can identify the dominant frequency components depending on thrust. The underlying method is described in detail in the document [Spectrogram Analysis](../Spectrum_Spectogram/Spectogram_Analysis.md).
