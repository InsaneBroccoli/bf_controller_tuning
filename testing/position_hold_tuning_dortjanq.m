...
dec        = round((1/Ts_log_raw) / 10);                                                %THIS HAS TO CHANGE
data       = data(1:dec:end, :);
...
figure(6)
bode(T, 'k', omega_bode, opt); 
hold on
bode(CL_ana.T, 'r', omega_bode, opt);
grid on
title('Comparison Closed Loop')
